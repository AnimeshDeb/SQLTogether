...
export default function QuestPage() {
  ...
  // 🌟 FILTER: Group only the quests that belong to the current category/lesson
  const currentCategoryQuests = quest
    ? quests.filter((q) => q.category === quest.category)
    : [];

  // 🌟 Calculate Navigation Indices based ONLY on this category
  const currentIndex = currentCategoryQuests.findIndex((q) => q.id === id);
  const hasPrevious = currentIndex > 0;
  const hasNext = currentIndex < currentCategoryQuests.length - 1;
  const prevQuest = hasPrevious
    ? currentCategoryQuests[currentIndex - 1]
    : null;
  const nextQuest = hasNext ? currentCategoryQuests[currentIndex + 1] : null;
  ...
  // 🌟 NEW: Reset state when the user navigates to a new problem without refreshing
  useEffect(() => {
    const savedCode = localStorage.getItem(`quest_${id}_code`);
  ...
  useEffect(() => {
    ...
    const setupDatabase = async () => {
      ...
      try {
        ...
        const schemaResult = await dbInstance.query(
        ...
        const tablePromises = schemaRows.map(async (row) => {
          ...
          const tableData = await dbInstance.query(
        ...
        const extractedTables = results.reduce(
          (acc, curr) => {
  ...
  const handleSubmitAnswer = async () => {
    ...
    try {
      ...
      const expected = (
        typeof quest.expected_output === 'string'
      ...
      if (actualRows.length !== expected.length) {
        setQueryError(
          `Row count mismatch: Expected ${expected.length} rows, but got ${actualRows.length}.`,
        );
        return;
      }
      ...
      if (
        ...
        setQueryError(
          'Output does not match the expected result. Look closely at the required columns and rows!',
        );
      }
    } catch (err: unknown) {
      if (err instanceof Error) setQueryError(err.message);
    ...
    <div className="min-h-screen bg-[#0f111a] text-slate-100 p-6 flex flex-col gap-6">
      ...
      <div className="w-full flex justify-between items-center">
        ...
        <div className="flex gap-4">
          <button
            onClick={() => prevQuest && navigate(`/quest/${prevQuest.id}`)}
          ...
          <button
            onClick={() => nextQuest && navigate(`/quest/${nextQuest.id}`)}
            disabled={!hasNext}
            className="text-slate-400 text-sm font-bold hover:text-emerald-400 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
          >
            Next →
      ...
      <div className="flex flex-col xl:flex-row gap-6 flex-1">
        <div className="w-full xl:w-1/2 flex flex-col gap-6">
          <div className="bg-[#141620] border border-slate-800 rounded-xl p-6 shadow-lg relative overflow-hidden">
            {alreadySolved && (
              <div className="absolute top-0 right-0 bg-emerald-500/10 border-b border-l border-emerald-500/30 px-4 py-1 rounded-bl-lg text-xs font-bold text-emerald-400">
                ✓ COMPLETED
              </div>
            )}
            ...
            <h1 className="text-2xl font-bold text-white mt-1 mb-4">
              {quest.title}
            </h1>
                    ...
                    {rows.map((r, i) => (
                      <tr key={i}>
                        {Object.values(r).map((v, j) => (
                          <td
                            key={j}
                            className="px-4 py-2 text-slate-300 font-mono"
                          >
                            {String(v)}
                          </td>
                        ))}
        ...
        <div className="w-full xl:w-1/2 flex flex-col gap-6">
          ...
          <div
            className={`bg-[#141620] border ${isPassed ? 'border-emerald-500 shadow-[0_0_20px_rgba(16,185,129,0.15)]' : 'border-slate-800'} rounded-xl p-6 flex-1 flex flex-col relative min-h-[300px]`}
          >
            <div className="flex justify-between items-start mb-4">
              ...
              {isPassed && (
                <div className="flex items-center gap-4 bg-emerald-500/10 border border-emerald-500/30 px-4 py-2 rounded-lg z-10">
                  ...
                  {/* 🌟 UPGRADED SUCCESS BANNER: Auto-routes to next problem if one exists */}
                  {hasNext ? (
                    <button
                      onClick={() =>
                        nextQuest && navigate(`/quest/${nextQuest.id}`)
                      }
                      className="bg-emerald-500 text-slate-900 text-xs font-bold px-4 py-2 rounded hover:bg-emerald-400 transition-colors"
                    >
                      Next Quest ➔
                    </button>
                  ) : (
            ...
            <div className="flex-1 overflow-auto">
                  ...
                  <thead className="bg-slate-900/80 text-slate-500">
                    <tr>
                      {queryResult.length > 0 &&
                        Object.keys(queryResult[0]).map((c) => (
                          <th key={c} className="px-4 py-2">
                            {c}
                          </th>
                        ))}
              ...
              {queryResult && queryResult.length === 0 && !queryError && (
                <div className="text-slate-500 italic text-sm mt-4">
