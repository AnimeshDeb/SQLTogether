        ...
        <div className="flex items-center gap-4">
          <div className="w-10 h-10 bg-emerald-500 rounded-lg flex items-center justify-center shadow-[0_0_15px_rgba(16,185,129,0.4)]">
            ...
            <svg className="w-6 h-6 text-slate-900" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
              <path d="M12 21c-4.97 0-9-3.134-9-7a6.98 6.98 0 0 1 1.73-4.57L3 4.5 8.07 6.3A10.36 10.36 0 0 1 12 5.5c1.42 0 2.78.21 4 .58L21 4.5l-1.73 4.93A6.98 6.98 0 0 1 21 14c0 3.866-4.03 7-9 7z" fill="currentColor" />
              <path d="M8 13.5c.5.5 1.5.5 2 0M14 13.5c.5.5 1.5.5 2 0" stroke="#0f111a" strokeWidth="2" />
              <polygon points="12,15 11,14 13,14" fill="#0f111a" />
            </svg>
          </div>
          <div>
      ...
      <div className="max-w-6xl mx-auto flex flex-col lg:flex-row gap-12 relative">
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3">
                    <svg className={`w-4 h-4 ${isActive ? 'text-emerald-400' : 'text-slate-500 group-hover:text-slate-400'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div className="flex-1 max-w-4xl space-y-24">
          {categories.map((category) => {
            ...
            return (
              <div key={category} id={category} className="relative module-section scroll-mt-24 lg:scroll-mt-8">
                <div className="flex items-center gap-4 mb-8">
                  <div className="h-px flex-1 bg-gradient-to-r from-transparent to-slate-800"></div>
                  <h2 className="text-2xl font-black text-white tracking-widest uppercase">MODULE: {category}</h2>
                  <div className="h-px w-8 lg:flex-1 bg-gradient-to-l from-transparent to-slate-800"></div>
                </div>

                <h3 className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-4 pl-2">Step 1: Interactive Lesson</h3>
                <div
                  ...
                  className={`mb-10 group relative bg-[#141620] border rounded-xl p-6 cursor-pointer transition-all duration-300 hover:-translate-y-1 hover:shadow-xl flex justify-between items-center ${
                    isLessonDone ? 'border-emerald-500/50 hover:border-emerald-400' : 'border-slate-800 hover:border-emerald-500/50'
                  }`}
                >
                  <div>
                    <div className="flex flex-wrap items-center gap-3 mb-1">
                      <span className={`text-xs font-bold uppercase tracking-widest ${isLessonDone ? 'text-emerald-500' : 'text-slate-400'}`}>
                        {category} CONCEPT
                      </span>
                    </div>
                  ...
                  <div className={`w-10 h-10 shrink-0 rounded-full flex items-center justify-center border transition-colors ${isLessonDone ? 'bg-emerald-500/10 border-emerald-500/30' : 'bg-slate-800 border-slate-700 group-hover:bg-emerald-500 group-hover:border-emerald-400'}`}>
                    <svg className={`w-5 h-5 ${isLessonDone ? 'text-emerald-400' : 'text-slate-400 group-hover:text-slate-900'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      {isLessonDone ? <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" /> : <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14 5l7 7m0 0l-7 7m7-7H3" />}
                    </svg>
                  </div>
                </div>
                ...
                {categoryQuests.length > 0 ? (
                  <>
                    <h3 className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-4 pl-2">Step 2: SQL Problems</h3>
                        ...
                        return (
                          <div
                            ...
                            className={`group relative bg-[#141620]/50 border rounded-lg p-5 cursor-pointer transition-all hover:-translate-y-1 ${
                              isSolved ? 'border-emerald-500/30 hover:border-emerald-400' : 'border-slate-800 hover:border-slate-600'
                            }`}
                          >
                            {isSolved && (
                            ...
                            <h4 className="text-md font-bold text-white mb-2 pr-6">{quest.title}</h4>
                            <p className="text-slate-400 text-xs line-clamp-2">{quest.prompt}</p>
                          </div>
                        );
                      })}
                    </div>
                  ...
                  isCoreConcepts && (
                    <div className="pl-2 sm:pl-4 sm:border-l-2 border-indigo-500/30 sm:ml-2 py-4">
                      <p className="text-slate-400 text-sm italic flex items-center gap-2">
                        ...
                        This module is a study guide. There are no coding problems required!
                      </p>
                    </div>
                  )
                )}
